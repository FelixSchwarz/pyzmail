...
class TestParse(unittest.TestCase):
    def test_decode_mail_header(self):
        ...
        self.assertEqual(decode_mail_header('hello '), u'hello ')
        self.assertEqual(
            decode_mail_header('=?iso-8859-1?q?Courrier_=E8lectronique_Fran=E7ais?='),
            u'Courrier \xe8lectronique Fran\xe7ais',
        )
        self.assertEqual(
            decode_mail_header('=?utf8?q?Courrier_=C3=A8lectronique_Fran=C3=A7ais?='),
            u'Courrier \xe8lectronique Fran\xe7ais',
        )
        self.assertEqual(decode_mail_header('=?utf-8?b?RnJhbsOnYWlz?='), u'Fran\xe7ais')
        self.assertEqual(
            decode_mail_header(
                '=?iso-8859-1?q?Courrier_=E8lectronique_?= =?utf8?q?Fran=C3=A7ais?='
            ),
            u'Courrier \xe8lectronique Fran\xe7ais',
        )
        self.assertEqual(
            decode_mail_header(
                '=?iso-8859-1?q?Courrier_=E8lectronique_?= =?utf-8?b?RnJhbsOnYWlz?='
            ),
            u'Courrier \xe8lectronique Fran\xe7ais',
        )
        if six.PY3:
            self.assertEqual(
                decode_mail_header(
                    'h_subject_q_iso_8858_1 : =?ISO-8859-1?Q?Fran=E7ais=E20accentu=E9?= !'
                ),
                u'h_subject_q_iso_8858_1 : Fran\xe7ais\xe20accentu\xe9 !',
            )
        else:
    ...
    def test_get_mail_addresses(self):
        ...
        self.assertEqual(
            ...
            get_mail_addresses(Msg('foo@example.com'), 'to'),
        )
        self.assertEqual(
            [
                ('Foo', 'foo@example.com'),
            ],
            get_mail_addresses(Msg('Foo <foo@example.com>'), 'to'),
        )
        ...
        self.assertEqual(
            ...
            get_mail_addresses(Msg('foo@example.com , bar@example.com'), 'to'),
        )
        self.assertEqual(
            [('Foo', 'foo@example.com'), ('Bar', 'bar@example.com')],
            get_mail_addresses(
                Msg('Foo <foo@example.com> , Bar <bar@example.com>'), 'to'
            ),
        )
        self.assertEqual(
            [('Foo', 'foo@example.com'), ('bar@example.com', 'bar@example.com')],
            get_mail_addresses(Msg('Foo <foo@example.com> , bar@example.com'), 'to'),
        )
        self.assertEqual(
            [('Mr Foo', 'foo@example.com'), ('bar@example.com', 'bar@example.com')],
            get_mail_addresses(
        ...
        self.assertEqual(
            [(u'Beno\xeet', 'benoit@example.com')],
            get_mail_addresses(
                Msg('=?utf-8?q?Beno=C3=AEt?= <benoit@example.com>'), 'to'
            ),
        )
        ...
        if six.PY2:
            self.assertEqual(
                [(u'Ant\ufffd\ufffdnio Foo', 'a.foo@example.com')],
                get_mail_addresses(Msg(address), 'to'),
            )
        else:
            ...
            self.assertEqual(
                ...
                get_mail_addresses(
                    Msg(
                        email.header.Header(
                            address, charset=email.charset.UNKNOWN8BIT, header_name='to'
                        )
                    ),
                    'to',
                ),
    ...
    def test_get_filename(self):
        ...
        attach.add_header(
            'Content-Disposition',
            'attachment',
            filename=('iso-8859-1', 'fr', encoded_filename),
        )
...
"""
        ...
        self.assertEqual(
            parts[0].is_body, 'text/plain'
        )  # not a error, is_body must be type
...
'''
    ...
    def check_message_1(self, msg):
        ...
        self.assertEqual(
            msg.get_addresses('to'), [(u'A', 'a@foo.com'), (u'B', 'b@foo.com')]
        )
        self.assertEqual(
            msg.get_addresses('cc'), [(u'C', 'c@foo.com'), (u'd@foo.com', 'd@foo.com')]
        )
...
"""
    ...
    def check_message_2(self, msg):
        self.assertEqual(
            msg.get_subject(), u'contains 8bits attachments using different encoding'
        )
    ...
    def check_message_3(self, msg):
        ...
        self.assertEqual(
            to[0][0], u'Ant\ufffd\ufffdnio Foo' if six.PY2 else u'Ant??nio Foo'
        )
