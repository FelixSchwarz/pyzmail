    ...
    def test_compose_and_parse(self):
        ...
        for mailpart in msg.mailparts:
            if mailpart.is_body:
                ...
                self.assertTrue(
                    isinstance(mailpart.sanitized_filename, six.string_types)
                )
                if mailpart.type == 'text/plain':
                    self.assertEqual(
                        mailpart.get_payload(), text_content.encode(text_encoding)
                    )
                else:
                ...
                found = False
                for attach in lst:
                    found = (mailpart.filename and attach[3] == mailpart.filename) or (
                        mailpart.content_id and attach[3] == mailpart.content_id
                    )
                    if found:
                        break
                ...
                if found:
                    ...
                    if (
                        attach[1] == 'text'
                        and attach[4]
                        and isinstance(attach[0], six.text_type)
                    ):
